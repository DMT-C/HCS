...
def Load_data(data_name, t1_path, t2_path, gt_path, patch_size, mode):
    ...
    if data_name == "bastrop":
        ...
        mat = io.loadmat(t1_path)
        # t1 = mat['t1_L8_clipped'][:, :, 0]
        # t2 = mat["logt2_clipped"][:, :, 3]
        t1 = mat['logt2_clipped'][:, :, 0]
        t2 = mat["t1_L8_clipped"][:, :, 3]
        t1 = to_normalization(t1)
        t2 = to_normalization(t2)
        gt = mat["ROI"]
        ...
        t1 = cv2.imread(t1_path)[:, :, 2]
        t2 = cv2.imread(t2_path)[:, :, 2]
        #t1 = np.mean(t1, axis=2, keepdims=True)
        #t2 = np.mean(t2, axis=2, keepdims=True)
        gt = cv2.imread(gt_path)[:, :, 0]
